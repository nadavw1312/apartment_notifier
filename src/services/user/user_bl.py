...
from src.services.user.user_dal import add_user, update_user_preferences
...
async def update_user_preferences(session: AsyncSession, user_id: int, min_price: int | None, max_price: int | None, min_area: int | None, max_area: int | None, min_rooms: int | None, max_rooms: int | None):
    return await update_user_preferences(session, user_id, min_price, max_price, min_area, max_area, min_rooms, max_rooms)